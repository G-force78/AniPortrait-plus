...
def upscale_video_with_face_enhancer(video_path, method):
  ...
  with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as temp_file:
    output_video = ffmpeg.output(
        input_video.video, temp_file.name, vcodec='libx264', r=30
    )
    ffmpeg.run(output_video)

  # Use ffmpeg to add the original audio to the upscaled video
  final_video_path = f"{os.path.splitext(video_path)[0]}_upscaled_with_audio.mp4"
  ffmpeg.input(temp_file.name).input(video_path).output(
      final_video_path, acodec='copy', vcodec='copy'
  ).run()
