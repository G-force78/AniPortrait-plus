...
def upscale_video_with_face_enhancer(video_path, method, device):
    # Load the face enhancement model
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Upscale the frames using the face enhancement model
        upscaled_frames = []
        for frame_path in tqdm(frame_paths, unit="frames", desc="Upscaling frames"):
            frame = cv2.imread(frame_path)
            if face_enhancer is None:
                upscaled_frame = face_enhancer_runner(frame, None)
            else:
                upscaled_frame = face_enhancer_runner(frame, face_enhancer)
        ...
        with tempfile.NamedTemporaryFile(suffix=".mp4", delete=True) as temp_file:
            ...
            # Move the temporary upscaled video to the final path
            os.rename(temp_file.name, final_video_path)
